...
Cela signifie qu'il n'est pas toujours possible, pour une installation unique de Python, de couvrir tous les besoins de toutes les applications. Basiquement, si une application A dépend de la version 1.0 et qu'une application B dépend de la version 2.0, ces dépendances entrent en conflit et installer la version 1.0 ou 2.0 laisse une des deux applications incapable de fonctionner sur le même environnement.  
...
* Connectez vous sur votre compte qui **Github** et créer un dépôt publique **streamlit_app**

* Allez sur **VSCODE**, ouvrez un terminal 

* Copiez le lien de votre dépôt pour le cloner.  

### Créer un nouvel environnement python
Maintenant que vous avez installé **python**, créons un **environnement virtuel python** pour gérer toutes les dépendances de la bibliothèque Python.  

Pour créer un nouvel environnement avec **Python**, saisissez ce qui suit sur votre terminal :  
...
* Créez un fichier ***requirements.txt** dans votre dépôt **streamlit_app**
...
Allez dans votre dépôt local et à l'interieur du dépôt créez un fichier **my_app.py**.  
...
Retournez sur le terminal et exécuter la commande suivante:

```
git status
```

* créez simplement un fichier nommé **.gitignore** avec votre éditeur de texte.

Le fichier **.gitignore** sert à dire à Git quels fichiers ou dossiers ne doivent pas être suivis ni envoyés dans le dépôt.

C’est utile pour éviter de versionner :

     - des fichiers temporaires,
     - des dépendances installées automatiquement,
     - des fichiers sensibles,
     - des fichiers générés à la compilation.

* Copiez le text suivant dans le fichier .gitignore et sauvegardez votre fichier
...
* Ouvrez votre fichier my_app.py Copiez les lignes de code suivantes dans votre fichier:  
...
* Ouvrez le fichier my_app.py Copiez les lignes de code suivantes dans votre fichier slt_multiselect_app.py:  
...
* Créez un nouveau fichier data_app.py Copiez les lignes de code suivantes dans votre fichier data_app.py:  
...
* Lancez votre application streamlit si nécessaire:  
...
Ouvrez le fichier data_app.py Copiez les lignes de code suivantes:
...
### Streamlit file_uploader :file_folder: :

Une application simple pour afficher charger un fichier

```
git branch file_uploader     # Créer une nouvelle branch

git branch            # vérifier que la nouvelle branch existe

git checkout file_uploader   # se positionner sur la nouvelle branch
```
...
git status
git add data_app.py
git commit -m "ajout d'une fonction de chargement de fichier"
git push origin file_uploader
```

* Faire une rapide analyse des données:

* Créer un nouveau fichier python titanic_data.py
...
```
git status
git add titanic_data.py
git commit -m "analyse données titanic"
git push origin file_uploader
```
